...
- 20% 할인해 드리고 있어요.
  - offer : 제공하다, 제안하다, 할인하다(서비스를 제공하는 뉘앙스)
  - discount : 할인
...
```
I would say it's the PT class.
```

- 제 생각에는 PT 수업인 것 같아요.
  - I would say : 제 생각에는 ~이다, ~라고 말하고 싶어요.
    - 단정적으로 말하기보다 한 번 생각해보고 부드럽게 대답할 때 사용한다.
...
```
It's a black leather bi-fold wallet.
```

- 검은색 가죽 이단 접이식 지갑입니다.
  - bi-fold : 이단 접이식, 두 번 접는
...
- 태국이 당신에게 가장 좋은 선택일 거예요.
  - best bet : 가장 좋은 선택, 가장 유력한 후보
...
- 넌 기억력이 형편없어!
  - memory like a sieve : 기억력이 나쁘다, 잘 잊어버린다. (체)구멍이 많은 것 처럼 기억이 빠져나감
...
- (물건) 거의 새것과 다름없는 상태예요.
- mint condition : 거의 새 것과 다름없는 상태, 최상의 상태
...
- 나 지금 무슨 생각하고 있었지? / 말하다가 흐름이 끊겼어.
- train of thought : 사고의 흐름
...
```
Try to job my memory.
```

- 내 기억을 되살려보려고 해봐. / 내 기억을 다시 생각해보려고 해봐.
  - job one's memory → ~의 기억을 되살리다, 기억을 더듬다
...
- 내 생각에 당신은 잘못된 나무를 향해 짖고 있는 것 같습니다.
  - 엉뚱한 사람을 비난하고 있다 / 헛다리 짚고 있다.
...
- 아이디어가 정말 많아.
- a million and one: 수없이 많은, 굉장히 많은
...
```
I'm tempted to get the new cell phone.
```

- 나는 새 휴대폰을 사고 싶어.
- be tempted to ~: ~하고 싶다, ~하고 싶은 마음이 들다  
...
- (직역) 그것은 더 나은 날들을 보아왔다 → (의역) 예전만 못하다.
  - have/has pp 의 현재 완료 문장 임.
  - 과거에는 좋았는데, 그게 완료되었다 라는 느낌이라서 과거 완료 had pp 가 쓰여야 할 것 같은데, 지금은 그러지 못하다... 라는 현재가 메인인 문장으로 현재 완료를 써야 한다고 하는데 이해는 안된다.
