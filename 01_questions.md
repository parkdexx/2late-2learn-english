...
```
By next month, the company ______ in the new headquarters for a full year.

A) operates
B) will operate
C) will have been operating
D) has been operating
```

### 풀이
- 해석: 다음 달이면 이 회사는 새 본사에서 1년 동안 운영되어 온 셈이 됩니다.
- 해설: By next month은 미래의 특정 시점을 나타냅니다. 그리고 for a full year는 그 시점까지의 기간을 나타내므로, **미래완료 진행** 시제인 (C) will have been operating이 가장 적절합니다. 이 시제는 특정 미래 시점(다음 달)에 어떤 동작이나 상태가 **얼마 동안 지속될지**를 강조할 때 사용됩니다.

### 분석
- (A) operates: 현재 시제 → 미래의 특정 시점까지의 기간을 나타내므로 부적절
- (B) will operate: 단순 미래 → '다음 달에 운영할 것이다'라는 단순 미래 시제이므로, '1년 동안 운영되어 온 셈'이라는 맥락(기간 지속)과 맞지 않음
- (C) will have been operating: 미래완료 진행 → '다음 달이면 (과거부터 지금까지) 1년 동안 운영되어 온 상태일 것이다'라는 의미로, 기간의 지속을 나타내므로 가장 적절
- (D) has been operating: 현재완료 진행 → '지금까지 운영해오고 있다'라는 의미로, 미래 시점(next month)과 맞지 않음
...
```
The supervisor said that the report ______ submitted by noon, but it arrived after 3 p.m.
...
- 해석: 관리자가 보고서가 정오까지 제출되어야 한다고 말했지만, 그것은 오후 3시 이후에 도착했다.
- 해설: 주절의 시제가 과거(said)이므로, that절의 내용도 과거 시제로 일치시켜야 한다. 비록 내용상 미래의 일이더라도, 말하는 시점이 과거이므로 '제출될 것이었다'는 과거 시제(was to be submitted)로 표현한다.

### 분석

- (A) is: 현재시제로, 과거 시점(said)에서 미래의 일을 나타낼 수 없다.
- (C) will be: 단순 미래시제로, 과거 시점에서 미래를 나타내는 간접화법 규칙(과거 시제 일치)에 맞지 않는다.
- (D) has been: 현재완료시제로, 과거 시점에서 미래를 나타내는 데 부적절하다.
...
```
The memo stated that the new policy ______ effect on May 1st.
...
- 그 메모는 새 정책이 5월 1일에 시행될 것이라고 밝혔다.
- 메모가 작성된 시점보다 미래에 시행될 정책을 말하고 있음
- 일반 미래를 표현하는 will + 동사원형이 와야 하는데
...
- (D) has taken → 현재완료는 현재 기준이라 부적절하다.
...
```
The new software update will allow users to customize their interface more ________ than the previous version.
...
- 빈칸은 '커스터마이징하다'라는 의미의 동사(customize)를 수식하는 자리입니다. 
- 영어에서 동사를 수식하여 '어떻게' 동작하는지를 설명하는 품사는 부사예요. 
...
- (A) efficient
  - 형용사로 '효율적인'이라는 뜻입니다. 
  - 형용사는 주로 명사를 수식하거나(an efficient system), 보어 자리(The system is efficient)에 쓰입니다. 
  - 이 문장에서는 동사인 customize를 꾸며줘야 하므로 형용사인 (A)는 올 수 없습니다.
- (B) efficiency
  - 명사로 '효율성'이라는 뜻입니다. 
  - 빈칸에 명사가 들어가면 '인터페이스 효율성을 커스터마이징하다'라는 의미가 되어야 하는데, 
  - 그렇게 되면 more와 than을 사용한 비교급 문장이 성립되지 않습니다. 
  - (명사의 비교급은 more efficiency 형태로 쓰일 수 있지만, 여기서는 '더 효율적으로 동사하다'라는 의미가 필요합니다.)
- (D) more efficient
  - 이미 빈칸 앞에 more가 있는데 또 more를 쓰면 more more efficient가 되어 중복됩니다. 
  - 또한 (A)와 마찬가지로 형용사 형태이기 때문에 동사를 수식할 수 없습니다.
